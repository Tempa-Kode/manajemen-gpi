...
@section("content")
                ...
                <div class="card shadow-sm border-0 h-100">
                    <div class="card-header bg-gradient-primary text-white p-4">
                        <h5 class="mb-0 text-white">
                            ...
                            Informasi Keluarga
                            <span
                                class="badge rounded-pill text-bg-{{ isset($data->tgl_keluar) ? "danger" : "success" }} text-white">{{ isset($data->tgl_keluar) ? "Keluar" : "Aktif" }}</span>
                        </h5>
                    </div>
                    <div class="card-body p-4">
                        <div class="row g-4">
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <div class="input-group">
                                        ...
                                        @if ($data->ayah)
                                            <span
                                                class="input-group-text bg-{{ $data->tgl_meninggal_ayah ? "danger" : "success" }} text-white">
                                                {{ $data->status_ayah }}
                                            </span>
                                        @endif
                                    </div>
                                    @if ($data->tgl_meninggal_ayah)
                                        <small class="text-muted">
                                            <i class="fas fa-calendar-alt me-1"></i>
                                            Meninggal: {{ $data->tgl_meninggal_ayah->format("d F Y") }}
                                        </small>
                                    @endif
                                </div>
                                <div class="form-group">
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <div class="input-group">
                                        ...
                                        @if ($data->ibu)
                                            <span
                                                class="input-group-text bg-{{ $data->tgl_meninggal_ibu ? "danger" : "success" }} text-white">
                                                {{ $data->status_ibu }}
                                            </span>
                                        @endif
                                    </div>
                                    @if ($data->tgl_meninggal_ibu)
                                        <small class="text-muted">
                                            <i class="fas fa-calendar-alt me-1"></i>
                            ...
                            </div> {{-- Tanggal Keluar Keanggotaan Jemaat --}}
                        ...
                        <div class="card shadow-sm border-0">
                            ...
                            <div class="card-body p-3">
                                <div class="d-grid gap-2">
                                    <a href="{{ route("sekolah-minggu.create") }}?keluarga={{ $data->id }}"
                                        class="btn btn-outline-primary btn-sm">
                                        <i class="fas fa-plus me-2"></i>Tambah Anak SM
                                    </a>
                                    <a href="{{ route("remaja.create") }}?keluarga={{ $data->id }}"
                                        class="btn btn-outline-success btn-sm">
                                        <i class="fas fa-plus me-2"></i>Tambah Remaja
                                    </a>
                                    <a href="{{ route("data-jemaat.print", $data->id) }}" target="_blank"
                                        class="btn btn-outline-info btn-sm">
                                        <i class="fas fa-print me-2"></i>Cetak Data
                                    </a>
                                    @if (!isset($data->tgl_keluar))
                                        <form action="{{ route("data-jemaat.keluar", $data->id) }}" method="POST"
                                            class="d-inline">
                                            @csrf
                                            @method("PUT")
                            ...
                            <div class="card-body p-3">
                                @if ($data->ayah)
                                    <div class="mb-3">
                                        <div class="d-flex justify-content-between align-items-center mb-2">
                                            <span class="text-muted">
                                                <i class="fas fa-male me-2"></i>Ayah ({{ $data->ayah }})
                                            </span>
                                            <span
                                                class="badge bg-{{ $data->tgl_meninggal_ayah ? "danger" : "success" }} rounded-pill">
                                                {{ $data->status_ayah }}
                                            </span>
                                        </div>
                                        <div class="d-grid gap-1">
                                            @if (!$data->tgl_meninggal_ayah)
                                                <button type="button" class="btn btn-outline-danger btn-sm"
                                                    data-bs-toggle="modal" data-bs-target="#ayahMeninggalModal">
                                                    <i class="fas fa-calendar-alt me-1"></i>Tandai Meninggal
                                                </button>
                                            @else
                                                <small class="text-muted mb-1">
                                                    <i class="fas fa-calendar-alt me-1"></i>
                                                    {{ $data->tgl_meninggal_ayah->format("d F Y") }}
                                                </small>
                                                <form action="{{ route("data-jemaat.reset-ayah", $data->id) }}"
                                                    method="POST">
                                                    @csrf
                                                    @method("PUT")
                                                    <button type="submit" class="btn btn-outline-success btn-sm w-100">
                                                        <i class="fas fa-undo me-1"></i>Ubah Status
                                                    </button>
                                                </form>
                                            @endif
                                        </div>
                                ...
                                @if ($data->ibu)
                                    <div class="mb-0">
                                        <div class="d-flex justify-content-between align-items-center mb-2">
                                            <span class="text-muted">
                                                <i class="fas fa-female me-2"></i>Ibu ({{ $data->ibu }})
                                            </span>
                                            <span
                                                class="badge bg-{{ $data->tgl_meninggal_ibu ? "danger" : "success" }} rounded-pill">
                                                {{ $data->status_ibu }}
                                            </span>
                                        </div>
                                        <div class="d-grid gap-1">
                                            @if (!$data->tgl_meninggal_ibu)
                                                <button type="button" class="btn btn-outline-danger btn-sm"
                                                    data-bs-toggle="modal" data-bs-target="#ibuMeninggalModal">
                                                    <i class="fas fa-calendar-alt me-1"></i>Tandai Meninggal
                                                </button>
                                            @else
                                                <small class="text-muted mb-1">
                                                    <i class="fas fa-calendar-alt me-1"></i>
                                                    {{ $data->tgl_meninggal_ibu->format("d F Y") }}
                                                </small>
                                                <form action="{{ route("data-jemaat.reset-ibu", $data->id) }}"
                                                    method="POST">
                                                    @csrf
                                                    @method("PUT")
                                                    <button type="submit" class="btn btn-outline-success btn-sm w-100">
                                                        <i class="fas fa-undo me-1"></i>Ubah ke Aktif
                                                    </button>
                                                </form>
                                            @endif
                                        </div>
                                ...
                                @if (!$data->ayah && !$data->ibu)
                                    <div class="text-center text-muted">
                                        ...
                                        <p class="mt-2 mb-0">Data orang tua belum diisi</p>
                                    </div>
                                @endif
                            </div>
                        </div>
                                    ...
                                    <div class="list-group-item d-flex justify-content-between align-items-center px-0">
                                        <div class="d-flex align-items-center">
                                            ...
                                            <div>
                                                <h6 class="mb-0">
                                                    <a
                                                        href="{{ route("sekolah-minggu.show", $sm->id) }}">{{ $sm->nama }}</a>
                                                    @if ($sm->tgl_meninggal)
                                                        <span class="badge bg-danger ms-2">Meninggal</span>
                                                    @endif
                                                </h6>
                                                <small class="text-muted">
                                                    {{ $sm->kelas ?? "Belum ada kelas" }}
                                                    @if ($sm->tgl_meninggal)
                                                        <br><i
                                                            class="fas fa-calendar-alt me-1"></i>{{ $sm->tgl_meninggal->format("d F Y") }}
                                                    @endif
                                                </small>
                                            </div>
                                        </div>
                                        <a href="{{ route("sekolah-minggu.show", $sm->id) }}"
                                            class="badge bg-{{ $sm->tgl_meninggal ? "danger" : "primary" }}">
                                            Detail
                                        </a>
                                    </div>
                                @endforeach
                                    ...
                                    <div class="list-group-item d-flex justify-content-between align-items-center px-0">
                                        <div class="d-flex align-items-center">
                                            ...
                                            <div>
                                                <h6 class="mb-0">
                                                    <a
                                                        href="{{ route("remaja.show", $remaja->id) }}">{{ $remaja->nama }}</a>
                                                    @if ($remaja->tgl_meninggal)
                                                        <span class="badge bg-danger ms-2">Meninggal</span>
                                                    @endif
                                                </h6>
                                                <small class="text-muted">
                                                    {{ $remaja->pendidikan ?? "Belum ada pendidikan" }}
                                                    @if ($remaja->tgl_meninggal)
                                                        <br><i
                                                            class="fas fa-calendar-alt me-1"></i>{{ $remaja->tgl_meninggal->format("d F Y") }}
                                                    @endif
                                                </small>
                                            </div>
                                        </div>
                                        <a href="{{ route("remaja.show", $remaja->id) }}"
                                            class="badge bg-{{ $remaja->tgl_meninggal ? "danger" : "success" }}">
                                            Detail
                                        </a>
                                    </div>
                                @endforeach
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="alert alert-warning">
                        ...
                        <strong>Peringatan!</strong> Tindakan ini akan menghapus semua data terkait termasuk data anak
    ...
    <!-- Modal Tanggal Meninggal Ayah -->
    <div class="modal fade" id="ayahMeninggalModal" tabindex="-1" aria-labelledby="ayahMeninggalModalLabel"
        aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="ayahMeninggalModalLabel">
                        <i class="fas fa-calendar-alt text-danger me-2"></i>
                        Tanggal Meninggal Ayah
                    </h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <form action="{{ route("data-jemaat.meninggal-ayah", $data->id) }}" method="POST">
                    @csrf
                    @method("PUT")
                    <div class="modal-body">
                        <div class="mb-3">
                            <label for="tgl_meninggal_ayah" class="form-label">Tanggal Meninggal</label>
                            <input type="date" class="form-control" id="tgl_meninggal_ayah" name="tgl_meninggal_ayah"
                                required max="{{ date("Y-m-d") }}">
                            <div class="form-text">Masukkan tanggal meninggal {{ $data->ayah ?? "ayah" }}</div>
                        </div>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Batal</button>
    ...
    <!-- Modal Tanggal Meninggal Ibu -->
    <div class="modal fade" id="ibuMeninggalModal" tabindex="-1" aria-labelledby="ibuMeninggalModalLabel"
        aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="ibuMeninggalModalLabel">
                        <i class="fas fa-calendar-alt text-danger me-2"></i>
                        Tanggal Meninggal Ibu
                    </h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <form action="{{ route("data-jemaat.meninggal-ibu", $data->id) }}" method="POST">
                    @csrf
                    @method("PUT")
                    <div class="modal-body">
                        <div class="mb-3">
                            <label for="tgl_meninggal_ibu" class="form-label">Tanggal Meninggal</label>
                            <input type="date" class="form-control" id="tgl_meninggal_ibu" name="tgl_meninggal_ibu"
                                required max="{{ date("Y-m-d") }}">
                            <div class="form-text">Masukkan tanggal meninggal {{ $data->ibu ?? "ibu" }}</div>
                        </div>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Batal</button>
